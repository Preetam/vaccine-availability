...
sqlite3 -cmd '.separator ", "' appointments.db 'with data as (select "* [" || location || "](" || link || ")" as location, date, count(*) || " slots" as count from appointments group by 1, 2) select location, group_concat(date, "; "), cast(sum(count) as integer) from data;' >> README.md
